<template>
  <div class="admin-products-container pa-6">
    ...
    <v-card class="filter-bar border mb-6 pa-4" variant="flat" rounded="lg">
      <v-row align="center" dense>
        <v-col cols="12" sm="6" md="4">
          <v-text-field v-model="search" label="Search products..." prepend-inner-icon="mdi-magnify" density="compact"
            variant="outlined" color="primary" flat hide-details clearable></v-text-field>
        </v-col>
        <v-col cols="12" sm="6" md="2">
          <v-select v-model="typeFilter" label="Package Type" :items="[
            { title: 'All', value: null },
            { title: 'Budget', value: 'Budget' },
            { title: 'Standard', value: 'Standard' },
            { title: 'Premium', value: 'Premium' }
          ]" density="compact" variant="outlined" color="primary" hide-details clearable></v-select>
        </v-col>
        <v-col cols="12" sm="6" md="2">
          <v-select v-model="locationTypeFilter" label="Location Type" :items="[
            { title: 'All', value: null },
        ...
        <v-col cols="12" sm="6" md="4" class="text-right">
          <v-btn color="primary" class="btn-primary text-none font-weight-medium px-4" prepend-icon="mdi-plus"
            rounded="md" elevation="0" @click="openAddDialog">
            Add Product
          </v-btn>
        </v-col>
      </v-row>
    ...
    <v-card class="enterprise-card border" variant="flat" rounded="lg">
      <v-data-table v-model:items-per-page="itemsPerPage" :headers="headers" :items="filteredProducts"
        :loading="loading" item-value="productId" class="premium-table" fixed-header height="550">
        ...
        <template #item.ticket="{ item }">
          <v-chip size="small" :color="item.ticket > 5 ? 'success' : 'warning'" variant="tonal"
        ...
        <template #item.actions="{ item }">
          <div class="d-flex align-center justify-end text-no-wrap">
            <v-btn icon variant="text" size="small" class="mr-1" @click="openImageDialog(item)">
              <v-badge :content="getImageCount(item)" :color="getImageBadgeColor(item)" location="top end" offset-x="2"
                offset-y="2">
      ...
      <v-card rounded="lg" class="pa-4">
        ...
        <v-card-text class="pa-2">
          <v-form ref="productForm">
            <v-row dense>
              <v-col cols="12">
                <v-text-field v-model="newProduct.title" label="Product Title *" variant="outlined" density="compact"
                  required></v-text-field>
              </v-col>
              <v-col cols="12" sm="6">
                <v-text-field v-model="newProduct.location" label="Location *" variant="outlined" density="compact"
                  required></v-text-field>
              </v-col>
              <v-col cols="12" sm="6">
                <v-select v-model="newProduct.locationType" label="Region Type *" :items="['DOMESTIC', 'INTERNATIONAL']"
                  variant="outlined" density="compact" required></v-select>
              </v-col>
              <v-col cols="12" sm="6">
                <v-select v-model="newProduct.type" label="Package Type *" :items="['Budget', 'Standard', 'Premium']"
                  variant="outlined" density="compact" required></v-select>
              </v-col>
              <v-col cols="12" sm="6">
                <v-text-field v-model.number="newProduct.amount" label="Price (MMK) *" type="number" variant="outlined"
                  density="compact" required></v-text-field>
              </v-col>
              <v-col cols="6" sm="3">
                <v-text-field v-model.number="newProduct.day" label="Days" type="number" variant="outlined"
                  density="compact"></v-text-field>
              </v-col>
              <v-col cols="6" sm="3">
                <v-text-field v-model.number="newProduct.night" label="Nights" type="number" variant="outlined"
                  density="compact"></v-text-field>
              </v-col>
              <v-col cols="12" sm="6">
                <v-text-field v-model="newProduct.groupSize" label="Group Size (e.g., 15 Pax)" variant="outlined"
                  density="compact"></v-text-field>
              </v-col>
              <v-col cols="12" sm="6">
                <v-text-field v-model="newProduct.meals" label="Meals Info" variant="outlined"
                  density="compact"></v-text-field>
              </v-col>
              <v-col cols="12" sm="6">
                <v-text-field v-model="newProduct.travelDate" label="Travel Date" type="date" variant="outlined"
                  density="compact"></v-text-field>
              </v-col>
              <v-col cols="12" sm="6">
                <v-text-field v-model.number="newProduct.ticket" label="Available Tickets" type="number"
                  variant="outlined" density="compact"></v-text-field>
              </v-col>
              <v-col cols="12">
                <v-select v-model="newProduct.transport" label="Transport" :items="['FLIGHT', 'CAR']" variant="outlined"
        ...
        <v-card-actions class="px-2 pt-4">
          ...
          <v-btn color="primary" variant="flat" class="text-none px-4" rounded="md" @click="saveProduct">Save
            Product</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
      ...
      <v-card class="premium-dialog" rounded="lg">
        <v-card-title class="d-flex justify-space-between align-center pa-6 border-bottom">
          <div>
            ...
            <p class="text-caption text-muted mb-0 mt-0.5">Select real image files for <strong>{{ imageForm.title
            }}</strong></p>
          </div>
          ...
          <v-row dense>
            ...
            <v-col cols="6" md="6">
              <v-card variant="outlined" class="pa-4 bg-white border d-flex align-center" rounded="lg">
                ...
                <div class="flex-grow-1">
                  <div class="text-subtitle-2 font-weight-bold text-slate-800 mb-1">Main Cover Photo</div>
                  <input type="file" id="file" ref="fileTwo" @change="changeImageTwo" class="file-input-anim" />
                </div>
              </v-card>
            </v-col>
            ...
            <v-col cols="6" md="6">
              <v-card variant="outlined" class="pa-4 bg-white border d-flex align-center" rounded="lg">
                <v-avatar size="90" rounded="lg" class="mr-4 border bg-grey-lighten-4">
                  <v-img v-if="imagePreviews.photoThree != null" :src="filePhotoThree" cover></v-img>
                  <v-icon v-else size="36" color="grey-darken-1">mdi-image-filter-hdr</v-icon>
                </v-avatar>
                <div class="flex-grow-1">
                  <div class="text-subtitle-2 font-weight-bold text-slate-800 mb-1">Main Cover Photo</div>
                  <input type="file" id="file" ref="fileThree" @change="changeImageThree" class="file-input-anim" />
                </div>
              </v-card>
            </v-col>
            <v-col cols="6" md="6">
              <v-card variant="outlined" class="pa-4 bg-white border d-flex align-center" rounded="lg">
                ...
                <div class="flex-grow-1">
                  <div class="text-subtitle-2 font-weight-bold text-slate-800 mb-1">Main Cover Photo</div>
                  <input type="file" id="file" ref="fileFour" @change="changeImageFour" class="file-input-anim" />
                </div>
              </v-card>
            </v-col>
            <!-- Sub Photos (4 Slots) -->
        ...
        <v-card-actions class="pa-4 bg-white justify-end">
          ...
          <v-btn color="primary" class="btn-primary text-none px-5" elevation="0" :loading="uploadingImages"
    ...
    <v-dialog v-model="deleteDialog" max-width="400" persistent>
      <v-card class="premium-dialog pa-2" rounded="lg">
        ...
        <v-card-actions class="pa-4 pt-0 justify-center">
          <v-btn variant="outlined" color="slate-600" class="text-none mr-2"
            @click="deleteDialog = false">Cancel</v-btn>
          <v-btn color="error" class="text-none px-4" variant="flat" rounded="md" elevation="0"
...
export default defineComponent({
  ...
  methods: {
    ...
    getPhotoOne:function(){
      ...
      PackageService
        .updateProductPhoto(formDataOne, this.imagePreviews.productId, 1)
        .then((response) => {
          // this.fetchProducts();
          // this.imageDialog = false;
        })
    ...
    getPhotoTwo:function(){
      ...
      PackageService
        .updateProductPhoto(formDataTwo, this.imagePreviews.productId, 2)
        .then((response) => {
          // this.fetchProducts();
          // this.imageDialog = false;
        })
    ...
    getPhotoThree:function(){
      ...
      PackageService
        .updateProductPhoto(formDataThree, this.imagePreviews.productId, 3)
        .then((response) => {
...
var formDataFour = new FormData();
      ...
      PackageService
        .updateProductPhoto(formDataFour, this.imagePreviews.productId, 4)
        .then((response) => {
          // this.fetchProducts();
          // this.imageDialog = false;
        })
